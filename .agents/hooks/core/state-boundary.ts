...
function detectVendor(input: Record<string, unknown>): Vendor {
  ...
  if (event === "UserPromptSubmit" && "session_id" in input && !("sessionId" in input)) {
    return "codex"
  }
 ...
 */
export function buildRecallQuery(projectDir: string, recentEvents: OmaEvent[], promptText?: string): string {
  const terms: string[] = []
...
export async function onBoundary(
  ...
  const boundary = !previous || previous.vendor !== vendor || previous.vendorSid !== vendorSid
  ...
  const facts: MemoryFact[] = recallQuery ? await recallFacts(recallQuery, 5) : []
...
export async function run(input: HookInput, ctx: HandlerCtx): Promise<HandlerResult | null> {
  ...
  const rendered = await onBoundary(projectDir, vendor, vendorSid, input.prompt, input.source === "compact")
