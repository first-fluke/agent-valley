...
export function buildTriggerPatterns(triggers: string[], lang: string, cjkScripts: string[]): RegExp[] {
...
export function matchSkills(
  ...
  matches.sort((a, b) => (b.score !== a.score ? b.score - a.score : a.name.localeCompare(b.name)))
...
export function filterFreshMatches(
  ...
  const alreadyInjected = new Set(current && now - current.timestamp <= SESSION_TTL_MS ? current.injected : [])
...
export function isPersistentWorkflowActive(projectDir: string, sessionId: string): boolean {
  const stateDir = join(projectDir, ".agents", "state")
  if (!existsSync(stateDir)) return false
  try {
    const files = readdirSync(stateDir)
    return files.some((f) => f.endsWith(`-state-${sessionId}.json`) && f !== "skill-sessions.json")
  } catch {
    return false
  }
}
...
export function findClaudeSlashSkill(name: string, projectDir: string): ClaudeSlashSkillEntry | null {
  const candidates = [
  ...
  for (const skillPath of candidates) {
    ...
    return {
      name,
      skillRelPath: posixPath.startsWith(`${posixRoot}/`) ? posixPath.slice(posixRoot.length + 1) : posixPath,
      body: body.trim(),
    }
  }
  return null
