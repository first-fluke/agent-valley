...
export function readIndex(projectDir: string): StateIndex {
  ...
  try {
    const parsed = JSON.parse(readFileSync(path, "utf-8")) as Partial<StateIndex>
    return {
      schemaVersion: 1,
...
export function updateIndex(projectDir: string, mutate: (index: StateIndex) => void, maxRetries = 3): StateIndex {
  const path = indexPath(projectDir)
  ...
  process.stderr.write(`[oma] _index.json CAS retries exhausted (${maxRetries}); leaving it stale.\n`)
  process.stderr.write("[oma]   hint: run 'oma state repair' to re-derive active pointers\n")
  return readIndex(projectDir)
}

export function getActiveSid(index: StateIndex, category = "main"): string | null {
  return index.active[category] ?? index.active.main ?? null
}

export function setActiveSession(projectDir: string, category: string, sid: string): StateIndex {
  return updateIndex(projectDir, (index) => {
    index.active[category] = sid
  })
}

export function setLastSession(projectDir: string, vendor: string, vendorSid: string): StateIndex {
  return updateIndex(projectDir, (index) => {
