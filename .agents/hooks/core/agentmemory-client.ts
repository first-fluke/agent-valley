...
function extractTimestampMs(entry: SearchResult): number | null {
  ...
  const candidates: unknown[] = [obs.timestamp, obs.created_at, entry.timestamp, entry.created_at]
...
export function parseSearchResults(body: string, k: number, nowMs: number = Date.now()): RecalledFact[] {
  ...
  for (const entry of parsed.results as SearchResult[]) {
    ...
    const obs = entry.observation ?? {}
    const narrative = typeof obs.narrative === "string" && obs.narrative.trim() ? obs.narrative.trim() : ""
