...
function detectVendor(input: Record<string, unknown>): Vendor {
  ...
  if (process.env.KIRO_PROJECT_DIR || event === "stop" || hookEventName === "stop") {
    return "kiro"
  }

  // agy (Antigravity) Stop sends no hook_event_name; detect by stdin shape.
  if (isAgyInput(input)) return "antigravity"
  if (event === "Stop" && process.env.ANTIGRAVITY_PROJECT_DIR) return "antigravity"
  if (event === "Stop") {
...
export function deactivateAllForSession(projectDir: string, sessionId: string): void {
...
function incrementReinforcement(projectDir: string, workflow: string, sessionId: string, state: ModeState): void {
...
async function main() {
  ...
  const textToCheck = [input.prompt_response, input.response, input.content, input.message, input.transcript]
