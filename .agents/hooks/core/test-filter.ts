...
function detectVendor(input: Record<string, unknown>): Vendor {
  ...
  if (import.meta.filename.includes(`${join(".pi", "extensions")}`)) return "pi"
  ...
  if (event === "PreToolUse" && process.env.ANTIGRAVITY_PROJECT_DIR) return "antigravity"
  if (event === "PreToolUse") {
...
export async function run(input: HookInput, ctx: HandlerCtx): Promise<HandlerResult | null> {
  ...
  const filterScript = [getHookDir(vendor), join(".opencode", "plugins", "oma"), join(".agents", "hooks", "core")]
