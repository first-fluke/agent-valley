 ...
 * other vendor.
 */
function runCore(script: string, payload: Record<string, unknown>): Record<string, unknown> | null {
  try {
...
export default function omaHooks(pi: ExtensionAPI): void {
  ...
  pi.on("tool_call", async (event) => {
    ...
    const updated = (tf?.updatedInput as { command?: string } | undefined)?.command
